...
dist.dif <- read.csv2("results/distribution_differences.csv")
sk.dif <- read.csv2("results/sk_differences.csv")
...
dist.data.wide$year <- as.factor(dist.data.wide$year)
dist.data.wide$pool <- factor(dist.data.wide$pool, levels = my.pool)
dist.data.wide$trait <- factor(dist.data.wide$trait, levels = my.traits)
...
trait.dist.plot.full <- ggplot(trait.dist) + 
  # geom_density(aes(x = value, fill = pool), alpha = .5) +
  geom_histogram(aes(x = value, fill = pool), alpha = .9) +
  geom_text(data = dist.dif, 
            aes(x = Inf,y = Inf,label = sig.level),
...
sk.dif$label <- as.character(round(sk.dif$aSPU_pvalue,2))
sk.dif$label[sk.dif$label == "0"] <- "< 0.001"

sk.dif$label2 <- "n.s."
sk.dif$label2[sk.dif$aSPU_pvalue<0.01] <- "***"
...
dist.data.plot <- ggplot(dist.data.wide, aes(x = skewness_squared, y = kurtosis, label = year)) + 
  ...
  geom_text(data = sk.dif, 
            aes(x = Inf,y = Inf,label = label2),
            #vjust = "inward", hjust = "inward",
            hjust = 1.5,
            vjust = 1.5,size = 5) +
  # geom_label() +
